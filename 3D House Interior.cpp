...
#include<math.h>
#include<string.h>
bool insideRedCircle(float y, float z, float yc, float zc, float r)
{
    float dy = y - yc;
    float dz = z - zc;
    return (dy*dy + dz*dz) <= (r*r);
}
void drawPixelYZ(float x, float y, float z)
{
    glBegin(GL_POINTS);
    glVertex3f(x, y, z);
    glEnd();
...
void midpointCircleFilledYZ(float yc, float zc, float r, float x)
{
    float y = 0;
    float z = r;
    float p = 1 - r;
    ...
    while (y <= z)
    {
        for(float i = yc - y*0.01; i <= yc + y*0.01; i += 0.003)
        {
            drawPixelYZ(x, i, zc + z*0.01);
            drawPixelYZ(x, i, zc - z*0.01);
        }

        for(float i = yc - z*0.01; i <= yc + z*0.01; i += 0.003)
        {
            drawPixelYZ(x, i, zc + y*0.01);
            drawPixelYZ(x, i, zc - y*0.01);
        }
...
void bresenhamLineYZ(float y1, float z1, float y2, float z2, float x)
{
    float dy = abs(y2 - y1);
    float dz = abs(z2 - z1);

    float sy = (y1 < y2) ? 0.005 : -0.005;
    float sz = (z1 < z2) ? 0.005 : -0.005;
    ...
    while (true)
    {
        ...
        if (abs(y1 - y2) < 0.005 && abs(z1 - z2) < 0.005) break;
...
void redWallDesign()
{
    ...
    // position (upper + centered)
    float yc = 0.33;
    float zc = 0.35;

    float rRed   = 0.045;  // red radius
    float rBlack = 0.065;  // outer black
    ...
    for(int i = 0; i < 360; i++)
    {
        ...
        float y = yc + rRed * cos(a);
        float z = zc + rRed * sin(a);
        glVertex3f(x, y, z);
    }
    glEnd();
    ...
    {
        float a = i * 3.1416 / 180;
        float y = yc + rBlack * cos(a);
        float z = zc + rBlack * sin(a);
        glVertex3f(x, y, z);
    }
    glEnd();
    ...
    // ===== ADVANCED DESIGN (INSIDE ONLY RED) =====
    glColor3f(0.0, 0.0, 0.0);

    for(float i = -rRed; i <= rRed; i += 0.004)
    {
        for(float t = -rRed; t <= rRed; t += 0.002)
        {
            float y = yc + t;
            float z = zc + i;
            ...
            // 🔥 CLIP inside red circle
            if((dy*dy + dz*dz) <= (rRed*rRed))
                drawPixelYZ(x, y, z);
        }
...
void fanRotateFunc(int v)
{
    fanRotate2 += 5;
    if(fanRotate2 > 360) fanRotate2 = 0;

    glutPostRedisplay();
    glutTimerFunc(30, fanRotateFunc, 0);
}
void lines()
{
  glColor3f(0.0, 0.0, 0.0);   
...
void room1()
{
  ...
  // ================= FAN ROOM 1 =================
  glPushMatrix();
  glTranslatef(0.0, 0.48, 0.0);   // ceiling center
  glRotatef(fanRotate2, 0, 1, 0);
...
void room2()
{
  ...
  glBegin(GL_POLYGON);
  glVertex3f(0.99, 10000, 1);
  glVertex3f(0.99, 10000, 3);
  glVertex3f(0.99, 0.8, 3);
  glVertex3f(0.99, 0.8, 1);
  glEnd();
...
int main(int argc, char** argv)
{
  ...
  glutCreateWindow("3D House Interior");
  ...
  tfs(0);
  glutTimerFunc(0, fanRotateFunc, 0);
  glutTimerFunc(0, doort, 0);
